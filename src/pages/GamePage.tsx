...
export function GamePage() {
  ...
  useEffect(() => {
    ...
    if (mainWalletAddress && !iframeWalletSyncedRef.current) {
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      if (e.data?.type === "WALLET_CONNECTED" && e.data.address) {
        iframeWalletSyncedRef.current = true;
        setSlotsWalletAddress(e.data.address);
      }
  ...
  const handleDisconnectWallet = () => {
    try {
      iframeRef.current?.contentWindow?.postMessage({ type: "DISCONNECT_WALLET" }, "*");
      setSlotsWalletAddress(null);
    } catch {
      //
    }
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[var(--dashboard-bg)]">
      ...
      <header className="flex-shrink-0 hidden sm:flex items-center gap-3 px-4 py-3 border-b-2 border-[var(--dashboard-border)] bg-[var(--dashboard-surface)]">
        ...
        {isSlots ? (
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {slotsWalletAddress ? (
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl border-2 border-[var(--dashboard-border)] bg-[var(--dashboard-surface)]">
                <Wallet className="size-5 flex-shrink-0 text-emerald-400" aria-hidden />
                <span className="text-sm font-medium text-white truncate max-w-[120px]" title={slotsWalletAddress}>
                  {slotsWalletAddress.slice(0, 4)}…{slotsWalletAddress.slice(-4)}
                </span>
                <button
                  type="button"
                  onClick={handleDisconnectWallet}
                  className="text-xs font-medium text-[var(--dashboard-muted)] hover:text-white transition-colors"
                >
                  Disconnect
                </button>
              </div>
      ...
      {/* Mobile header (below sm): two rows so toggle isn't covered by music button */}
      <header className="flex-shrink-0 flex sm:hidden flex-col gap-2 px-3 py-2 border-b-2 border-[var(--dashboard-border)] bg-[var(--dashboard-surface)]">
        <div className="flex items-center gap-2 min-w-0">
          <Link
            to="/"
            state={{ returnTo: "casino" }}
            className="flex items-center gap-1.5 text-xs font-medium text-[var(--dashboard-muted)] hover:text-white transition-colors flex-shrink-0"
          >
            <ArrowLeft className="size-4" aria-hidden />
            Back
          </Link>
          ...
          {isSlots && (
            <button
              type="button"
              onClick={handleToggleMusic}
              className="p-1.5 rounded-lg text-[var(--dashboard-muted)] hover:text-white hover:bg-white/10 transition-colors flex-shrink-0"
              title="Toggle music"
              aria-label="Toggle music on or off"
            >
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className="size-4" aria-hidden>
                <path fillRule="evenodd" clipRule="evenodd" d="M21.6464 2.23699C21.8707 2.42699 22 2.70606 22 3.00001V16C22 18.2091 20.2091 20 18 20C15.7909 20 14 18.2091 14 16C14 13.7909 15.7909 12 18 12C18.7286 12 19.4117 12.1948 20 12.5351V4.18047L10 5.84713V18L9.99999 18.0032C9.99824 20.2109 8.20806 22 6 22C3.79086 22 2 20.2091 2 18C2 15.7909 3.79086 14 6 14C6.72857 14 7.41165 14.1948 8 14.5351V5.00001C8 4.51117 8.35341 4.09398 8.8356 4.01361L20.8356 2.01361C21.1256 1.96529 21.4221 2.04698 21.6464 2.23699ZM20 16C20 14.8954 19.1046 14 18 14C16.8954 14 16 14.8954 16 16C16 17.1046 16.8954 18 18 18C19.1046 18 20 17.1046 20 16ZM6 16C7.10457 16 8 16.8954 8 18C8 19.1046 7.10457 20 6 20C4.89543 20 4 19.1046 4 18C4 16.8954 4.89543 16 6 16Z" fill="currentColor"/>
              </svg>
            </button>
          )}
          ...
          <div className="flex items-center gap-2 w-full">
            {slotsWalletAddress ? (
              <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg border-2 border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] min-w-0 flex-1">
                <Wallet className="size-4 flex-shrink-0 text-emerald-400" aria-hidden />
                <span className="text-xs font-medium text-white truncate min-w-0 max-w-[100px]" title={slotsWalletAddress}>
                  {slotsWalletAddress.slice(0, 4)}…{slotsWalletAddress.slice(-4)}
                </span>
                <button
                  type="button"
                  onClick={handleDisconnectWallet}
                  className="text-xs font-medium text-[var(--dashboard-muted)] hover:text-white transition-colors flex-shrink-0"
                >
                  Disconnect
                </button>
              </div>
              ...
              <button
                type="button"
                onClick={handleConnectWallet}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium border-2 border-[var(--dashboard-border)] bg-[var(--dashboard-surface)] text-[var(--dashboard-text)] hover:border-[var(--dashboard-accent)]/60 hover:shadow-[0_0_16px_var(--dashboard-glow)] transition-colors w-full justify-center"
              >
                <Wallet className="size-4 flex-shrink-0" aria-hidden />
                Connect
              </button>
            )}
          </div>
