...
export function CasinoSection() {
  ...
  return (
    <div className="w-full flex-1 min-h-0 flex flex-col">
      <p className="text-[var(--dashboard-muted)] mb-2 text-center flex-shrink-0">
        Play using $KNUKL or $BUX to win NFT and token prizes
      </p>
      <h2 className="text-xl font-semibold text-white mb-4 text-center flex-shrink-0">Choose your game</h2>
          ...
          {GAMES.map(({ id, name, path, image, built }) => (
            <li key={id}>
              <button
                ...
                disabled={!built}
                className={`
                  w-full flex items-center justify-center gap-6 rounded-2xl border-2 p-5 transition-all
                  ${built
              ...
              >
                {failedImages.has(id) ? (
                  <div className="size-24 flex-shrink-0 rounded-lg bg-[var(--dashboard-border)] flex items-center justify-center text-[var(--dashboard-muted)] text-sm" aria-hidden>
                    ?
                  </div>
                ) : (
                  <img
                    src={image}
                    alt=""
                    className="size-24 object-contain flex-shrink-0 min-[840px]:size-28"
                    aria-hidden
                    onError={() => handleImageError(id)}
                  />
                )}
                {built ? (
                  <span className="text-xl font-medium text-white min-[840px]:text-2xl">{name}</span>
                ) : (
                  <div className="flex flex-col items-center">
                    <span className="text-xl font-medium text-white min-[840px]:text-2xl">{name}</span>
                    <span className="text-base text-[var(--dashboard-muted)] mt-1">Coming soon</span>
                  </div>
                )}
              </button>
            </li>
